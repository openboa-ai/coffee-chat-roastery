...
async function renamedPublicationFixture(): Promise<PublicationFixture> {
  const fixture = await publicationFixture();
  const baseSha = fixture.headSha;
  const renamedBeanId = "01890f3a-2b00-7000-8000-000000000002";
  const beanPath = `roastery/beans/${renamedBeanId}.md`;
  execFileSync("git", ["-C", fixture.root, "mv", fixture.beanPath, beanPath]);
  const beanBytes = fixture.beanBytes.replace(beanId, renamedBeanId);
  writeFileSync(join(fixture.root, beanPath), beanBytes);
  const { projectIndexBytes } = await import("../src/projection/index.js");
  const indexBytes = await projectIndexBytes(fixture.root);
  writeFileSync(join(fixture.root, "roastery", "index.json"), indexBytes);
  execFileSync("git", [
    "-C",
    fixture.root,
    "add",
