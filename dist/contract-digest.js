...
export function computeContractDigest(repositoryRoot) {
    try {
        ...
        let totalBytes = 0;
        for (const relativePath of [...CONTRACT_FILES].sort((left, right) => Buffer.from(left, "ascii").compare(Buffer.from(right, "ascii")))) {
            ...
            const content = readVerifiedFile(resolve(contractRoot, relativePath), ancestors, fileIdentities, "unsafe_contract_entry", MAX_CONTRACT_FILE_BYTES, "unsafe_contract_entry");
