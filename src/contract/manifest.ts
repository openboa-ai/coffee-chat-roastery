...
function isCanonicalBundlePath(value: unknown): value is string {
  if (
    typeof value !== "string" ||
...
export async function loadContractManifest(
  ...
  const source = await readFile(
    resolve(repositoryRoot, "contract/contract.json"),
    "utf8",
  );
  return parseContractManifest(JSON.parse(source));
}

export function contractBundlePaths(manifest: ContractManifest): string[] {
  return ["contract/contract.json", ...Object.values(manifest.files)].sort(
