...
export async function validateRepository(
  ...
  const manifestResult = validateRoasteryManifest(manifestValue);
...
async function validateRefreshBundle(
  ...
  try {
    actualDigest = await digestContractBundle(bundle.root);
  } catch {
    return { status: "rejected", reason: "bundle_byte_mismatch" };
  }
  ...
  try {
    return {
      ...bundle,
      ...(await interpretContractBundle(bundle.root, declarationBytes)),
    };
  } catch {
...
interface ContractRefreshEvidenceInput {
  ...
  protectedCanaryReceipt: { reference: string; digest: string };
}

export function buildContractRefreshEvidence(
  ...
  return {
    ...
    protected_canary_receipt: input.protectedCanaryReceipt,
  };
}
