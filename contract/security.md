...
Directory and file identities remain valid through the complete operation.
Contract entry names are byte-read portable ASCII. Canonical text must be
well-formed UTF-8; decoding never substitutes invalid bytes. Offline Origin
validation requires the input to equal its canonical URL serialization, contain
only valid percent escapes, use HTTPS, and contain a syntactically valid
