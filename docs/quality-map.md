...
| Keep publication governance lean                  | PR-only quality, dependency review, trusted secret scanning, and CodeQL remain exact; normal changes auto-merge with zero approvals while sensitive paths require the maintainer team | `ci:policy`, GitHub rules and checks                               | PR   |
